if do_GWS
    ...
    switch fluct_plot_type
        case 'line'    
            ...
            field_fluctuations = testDipoleMovement(eigenvectors(:,idx_to_plot(1)),n_movements,movement_stdev,...
            ...
            field_fluctuations = testDipoleMovement(eigenvectors(:,idx_to_plot(2)),n_movements,movement_stdev,...
                k0,alpha,z_full_2d,static_dipole_positions,moving_dipole_positions,z_sources,z_detectors);
            max_fluctuations = max(max(abs(field_fluctuations(:,:)).^2));
            for n = 1:size(field_fluctuations,2)
                plot(x_points./wavelength,(abs(field_fluctuations(:,n)).^2)./max_fluctuations,'Color',colors(random_color_idx(n),:))
            ...
            field_fluctuations = testDipoleMovement(eigenvectors(:,idx_to_plot(3)),n_movements,movement_stdev,...
                k0,alpha,z_full_2d,static_dipole_positions,moving_dipole_positions,z_sources,z_detectors);
            max_fluctuations = max(max(abs(field_fluctuations(:,:)).^2));
            for n = 1:size(field_fluctuations,2)
                plot(x_points./wavelength,(abs(field_fluctuations(:,n)).^2)./max_fluctuations,'Color',colors(random_color_idx(n),:))
    ...
    disp(['Testing movement with GWS fields took ' num2str(toc) ' seconds.']);
end