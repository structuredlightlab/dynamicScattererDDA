...
switch fluct_plot_type
    case 'line'
        ...
        field_fluctuations = testDipoleMovement(eigenvectors_dep(:,idx_to_plot(1)),n_movements,movement_stdev,...
        ...
        for n = 1:size(field_fluctuations,2)
            plot(y_detectors./wavelength,(abs(field_fluctuations(:,n)).^2)./max_fluctuations,'Color',colors(random_color_idx(n),:))
        end
        xlabel('y, \lambda');   ylabel('Normalised intensity')
        text(22,0.9,num2str(idx_to_plot(1)),'FontName','cmss10','FontSize',get(groot,'defaultAxesFontSize'),...
            'BackgroundColor',[1 1 1 0.5]);
        text(-27,0.89,['\xi = ' num2str(xi_dep(idx_to_plot(1)),'%.2f')],...
        ...
        field_fluctuations = testDipoleMovement(eigenvectors_dep(:,idx_to_plot(2)),n_movements,movement_stdev,...
            k0,alpha,z_full_2d,static_dipole_positions,moving_dipole_positions,z_sources,z_detectors);
        max_fluctuations = max(max(abs(field_fluctuations(:,:)).^2));
        for n = 1:size(field_fluctuations,2)
            plot(y_detectors./wavelength,(abs(field_fluctuations(:,n)).^2)./max_fluctuations,'Color',colors(random_color_idx(n),:))
        ...
        xlabel('y, \lambda');
        text(22,0.9,num2str(idx_to_plot(2)),'FontName','cmss10','FontSize',get(groot,'defaultAxesFontSize'),...
            'BackgroundColor',[1 1 1 0.5]);
        text(-27,0.89,['\xi = ' num2str(xi_dep(idx_to_plot(2)),'%.2f')],...
            'FontName','cmss10','FontSize',get(groot,'defaultAxesFontSize'),'BackgroundColor',[1 1 1 0.5],'EdgeColor','black');
        set(ha(2),'Position',[subplot_x+subplot_width+subplot_buffer subplot_y subplot_width subplot_height],...
            'YTick',[])
        
        axes(ha(3));  hold on;    box on;
        field_fluctuations = testDipoleMovement(eigenvectors_dep(:,idx_to_plot(3)),n_movements,movement_stdev,...
            k0,alpha,z_full_2d,static_dipole_positions,moving_dipole_positions,z_sources,z_detectors);
        max_fluctuations = max(max(abs(field_fluctuations(:,:)).^2));
        for n = 1:size(field_fluctuations,2)
            plot(y_detectors./wavelength,(abs(field_fluctuations(:,n)).^2)./max_fluctuations,'Color',colors(random_color_idx(n),:))
        ...
        xlabel('y, \lambda');
        text(22,0.9,num2str(idx_to_plot(3)),'FontName','cmss10','FontSize',get(groot,'defaultAxesFontSize'),...
            'BackgroundColor',[1 1 1 0.5]);
        text(-27,0.89,['\xi = ' num2str(xi_dep(idx_to_plot(3)),'%.2f')],...
            'FontName','cmss10','FontSize',get(groot,'defaultAxesFontSize'),'BackgroundColor',[1 1 1 0.5],'EdgeColor','black');
...
disp(['Testing movement with Deposition matrix fields took ' num2str(toc) ' seconds.']);