if do_slow_optm
    ...
    %Calculate the optimised field everywhere:
    optimised_field = source_amps.*exp(1i*optimized_phases);
    optimised_field = optimised_field./norm(optimised_field);
    
    [optimised_total_field, optimised_scattered_field,~,opt_pol_data] = DDA_true2D(...
        k0,alpha,z_full_2d,exclusion_radius,z_sources,optimised_field,z_dipole_positions,show_pol_plots);
    max_amp = max([max(abs(initial_total_field)) max(abs(optimised_total_field))]);
    max_int = max([max(abs(initial_total_field).^2) max(abs(optimised_total_field).^2)]);
    ...
    % fig_name = date + ', ' + num2str(n_dipoles) + ' dipoles, slow optimiser results';
    fig_name = ['slowOptInt_' color_scale];
    ...
    saveFigure(fig,fig_name,save_path,{'fig'},0);
     ...
     switch fluct_plot_type
        case 'line'    
            ...
            % hsv_colors = hsv(n_movements);
            hsv_colors = colormap('jet');
            colors = hsv_colors(randperm(256),:);
            colors = colors(1:n_movements,:);
            ...
            max_fluctuations = 1; %max(max(abs(field_fluctuations(:,:)).^2));
            ...
            xlabel('y, \lambda');   ylabel('Intensity')
            ...
            % text(-0.55*scale,0.89,['\xi = ' num2str(xi_GWS(idx_to_plot(2)),'%.2f')],...
                % 'FontName','cmss10','FontSize',get(groot,'defaultAxesFontSize'),'BackgroundColor',[1 1 1 0.5],'EdgeColor','black');
            set(ha(2),'Position',[subplot_x+subplot_width+2*subplot_buffer subplot_y subplot_width subplot_height],...
                'YTick',[])
            h = axis(ha(1));
            axis(h)
    ...
    fig_name = 'slowOptIntFluctuations';
