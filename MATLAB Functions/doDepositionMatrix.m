if do_deposition_matrix
    tic
    % Choose deposition area
    r_avoid = percent_area*max(dipole_area);

    % Select n_sources number of points within the deposition area randomly
    % to sample for deposition matrix (Simon did it this way to ensure a
    % square matrix)
    z_avoid = r_avoid.*(rand(n_sources,1).*exp(1i*2*pi.*rand(n_sources,1)));
    switch selection_method
        case 'random'
        z_avoid = [moving_dipole_positions(1) + movement_stdev.*(rand(round(n_sources/2),1).*exp(1i*2*pi.*rand(round(n_sources/2),1))); ...
            moving_dipole_positions(2) + movement_stdev.*(rand(floor(n_sources/2),1).*exp(1i*2*pi.*rand(floor(n_sources/2),1)))];
    end
    % Calculate transmission matrix to points inside deposition area
    transmission_deposition = complex(zeros(n_sources));
    for n = 1:n_sources
        probe_source = zeros(n_sources,1);
        probe_source(n) = 1;
        source_at_detectors = sourceAtDetectors(k0,probe_source,z_sources,z_avoid);
        field_inside = DDA_true2D_onlyOutputs(k0,alpha,z_avoid,z_sources,...
            probe_source,source_at_detectors,z_dipole_positions);
        transmission_deposition(:,n) = field_inside;
    end

    % Calculate deposition matrix, Z
    deposition_matrix = ctranspose(transmission_deposition)*transmission_deposition;
    
    [eigenvectors_dep,eigenvalues_dep] = eig(deposition_matrix);
    abs_eigvals_dep = (diag(abs(eigenvalues_dep)));

    
    idx_to_plot = round(linspace(1,length(abs_eigvals_dep),8));
    ...
    [dep_field,~] = DDA_true2D(k0,alpha,z_full_2d,exclusion_radius,...
        z_sources,eigenvectors_dep(:,idx_to_plot(1)),z_dipole_positions,show_pol_plots);
    imagesc(x_points/wavelength,x_points/wavelength,...
        abs(dep_field).^2./max(max(abs(dep_field(:,:)).^2)))
    ...
    axis image;    ylabel('y, \lambda');
    text(-0.55*scale,-0.5*scale,['abs(\nu) = ' num2str(abs_eigvals_dep(idx_to_plot(1)),3)],...
    ...
    text(-0.55*scale,0.5*scale,num2str(idx_to_plot(1)),'Color','black','BackgroundColor',[1 1 1 0.5],...
        'FontName','cmss10','FontSize',get(groot,'defaultAxesFontSize'));
    set(ha(1),'Position',[subplot_x subplot_y subplot_height subplot_height],...
        'YDir','reverse','XTick',[],'ColorScale',color_scale)
    ...
    [dep_field,~] = DDA_true2D(k0,alpha,z_full_2d,exclusion_radius,...
        z_sources,eigenvectors_dep(:,idx_to_plot(2)),z_dipole_positions,show_pol_plots);
    imagesc(x_points/wavelength,x_points/wavelength,...
        abs(dep_field).^2./max(max(abs(dep_field(:,:)).^2)))
    plot(real(z_dipole_positions)/(wavelength),...
        imag(z_dipole_positions)/(wavelength),'LineStyle','none',...
    ...
    plot(real(moving_dipole_positions)/(wavelength),...
        ...
        'Marker','o','MarkerSize',7,'Color',col6)
    axis image
    text(-0.55*scale,-0.5*scale,['abs(\nu) = ' num2str(abs_eigvals_dep(idx_to_plot(2)),3)],...
        'Color','black','BackgroundColor',[1 1 1 0.5],...
        'FontName','cmss10','FontSize',get(groot,'defaultAxesFontSize'));
    ...
    [dep_field,~] = DDA_true2D(k0,alpha,z_full_2d,exclusion_radius,...
        z_sources,eigenvectors_dep(:,idx_to_plot(3)),z_dipole_positions,show_pol_plots);
    imagesc(x_points/wavelength,x_points/wavelength,...
        abs(dep_field).^2./max(max(abs(dep_field(:,:)).^2)))
    plot(real(z_dipole_positions)/(wavelength),...
        imag(z_dipole_positions)/(wavelength),'LineStyle','none',...
    ...
    plot(real(moving_dipole_positions)/(wavelength),...
        ...
        'Marker','o','MarkerSize',7,'Color',col6)
    axis image
    text(-0.55*scale,-0.5*scale,['abs(\nu) = ' num2str(abs_eigvals_dep(idx_to_plot(3)),3)],...
        'Color','black','BackgroundColor',[1 1 1 0.5],...
        'FontName','cmss10','FontSize',get(groot,'defaultAxesFontSize'));
    ...
    [dep_field,~] = DDA_true2D(k0,alpha,z_full_2d,exclusion_radius,...
        z_sources,eigenvectors_dep(:,idx_to_plot(4)),z_dipole_positions,show_pol_plots);
    imagesc(x_points/wavelength,x_points/wavelength,...
        abs(dep_field).^2./max(max(abs(dep_field(:,:)).^2)))
    plot(real(z_dipole_positions)/(wavelength),...
        imag(z_dipole_positions)/(wavelength),'LineStyle','none',...
    ...
    plot(real(moving_dipole_positions)/(wavelength),...
        ...
        'Marker','o','MarkerSize',7,'Color',col6)
    axis image
    text(-0.55*scale,-0.5*scale,['abs(\nu) = ' num2str(abs_eigvals_dep(idx_to_plot(4)),3)],...
        'Color','black','BackgroundColor',[1 1 1 0.5],...
        'FontName','cmss10','FontSize',get(groot,'defaultAxesFontSize'));
    ...
    [dep_field,~] = DDA_true2D(k0,alpha,z_full_2d,exclusion_radius,...
        z_sources,eigenvectors_dep(:,idx_to_plot(5)),z_dipole_positions,show_pol_plots);
    imagesc(x_points/wavelength,x_points/wavelength,...
        abs(dep_field).^2./max(max(abs(dep_field(:,:)).^2)))
    plot(real(z_dipole_positions)/(wavelength),...
        imag(z_dipole_positions)/(wavelength),'LineStyle','none',...
    ...
    text(-0.55*scale,-0.5*scale,['abs(\nu) = ' num2str(abs_eigvals_dep(idx_to_plot(5)),3)],...
        'Color','black','BackgroundColor',[1 1 1 0.5],...
        'FontName','cmss10','FontSize',get(groot,'defaultAxesFontSize'));
    ...
    [dep_field,~] = DDA_true2D(k0,alpha,z_full_2d,exclusion_radius,...
        z_sources,eigenvectors_dep(:,idx_to_plot(6)),z_dipole_positions,show_pol_plots);
    imagesc(x_points/wavelength,x_points/wavelength,...
        abs(dep_field).^2./max(max(abs(dep_field(:,:)).^2)))
    plot(real(z_dipole_positions)/(wavelength),...
        imag(z_dipole_positions)/(wavelength),'LineStyle','none',...
    ...
    axis image
    xlabel('x, \lambda'); 
    text(-0.55*scale,-0.5*scale,['abs(\nu) = ' num2str(abs_eigvals_dep(idx_to_plot(6)),3)],...
        'Color','black','BackgroundColor',[1 1 1 0.5],...
        'FontName','cmss10','FontSize',get(groot,'defaultAxesFontSize'));
    ...
    [dep_field,~] = DDA_true2D(k0,alpha,z_full_2d,exclusion_radius,...
        z_sources,eigenvectors_dep(:,idx_to_plot(7)),z_dipole_positions,show_pol_plots);
    imagesc(x_points/wavelength,x_points/wavelength,...
        abs(dep_field).^2./max(max(abs(dep_field(:,:)).^2)))
    plot(real(z_dipole_positions)/(wavelength),...
        imag(z_dipole_positions)/(wavelength),'LineStyle','none',...
    ...
    axis image
    xlabel('x, \lambda');
    text(-0.55*scale,-0.5*scale,['abs(\nu) = ' num2str(abs_eigvals_dep(idx_to_plot(7)),3)],...
        'Color','black','BackgroundColor',[1 1 1 0.5],...
        'FontName','cmss10','FontSize',get(groot,'defaultAxesFontSize'));
    ...
    [dep_field,~] = DDA_true2D(k0,alpha,z_full_2d,exclusion_radius,...
        z_sources,eigenvectors_dep(:,idx_to_plot(8)),z_dipole_positions,show_pol_plots);
    imagesc(x_points/wavelength,x_points/wavelength,...
        abs(dep_field).^2./max(max(abs(dep_field(:,:)).^2)))
    ...
    axis image
    xlabel('x, \lambda'); 
    text(-0.55*scale,-0.5*scale,['abs(\nu) = ' num2str(abs_eigvals_dep(idx_to_plot(8)),3)],...
        'Color','black','BackgroundColor',[1 1 1 0.5],...
        'FontName','cmss10','FontSize',get(groot,'defaultAxesFontSize'));
    ...
    disp(['Calculating and plotting deposition matrix took ' num2str(toc) ' seconds.']);
